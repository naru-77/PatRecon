...
parser.add_argument("--exp", type=int, default=1, help="experiments index")
parser.add_argument(
    "--seed",
    type=int,
    default=1,
    metavar="N",
    help="manual seed for GPUs to generate random numbers",
)
parser.add_argument(
    "--num-views", type=int, default=1, help="number of views/projections in inputs"
)
parser.add_argument(
    "--input-size", type=int, default=128, help="dimension of input view size"
)
parser.add_argument(
    "--output-size", type=int, default=128, help="dimension of ouput 3D model size"
)
parser.add_argument(
    "--output-channel", type=int, default=0, help="dimension of ouput 3D model size"
)
parser.add_argument(
    "--start-slice", type=int, default=0, help="the idx of start slice in 3D model"
)
parser.add_argument(
    "--test", type=int, default=1, help="number of total testing samples"
)
parser.add_argument(
    "--vis_plane", type=int, default=0, help="visualization plane of 3D images: [0,1,2]"
)
...
class MedReconDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        images = np.zeros(
            (args.input_size, args.input_size, args.num_views), dtype=np.uint8
        )  ### input image size (H, W, C)
        ### load image
        # 各投影画像を読み込み、配列に格納
...
def main():
    ...
    for idx in range(args.test):
        ...
        print(
            "Evaluate testing sample {} *********************************".format(
                idx + 1
            )
        )
...
def getErrorMetrics(im_pred, im_gt, mask=None):
    ...
    # mse_value = compare_mse(im_true=im_gt, im_test=im_pred)
    mse_value = skimage_mse(im_gt, im_pred)
    rmse_pred = np.sqrt(mse_value)
    # PSNR
    ...
    print(
        "mae: {mae_pred:.4f} | mse: {mse_pred:.4f} | rmse: {rmse_pred:.4f} | psnr: {psnr_pred:.4f} | ssim: {ssim_pred:.4f}".format(
            mae_pred=mae_pred,
            mse_pred=mse_pred,
            rmse_pred=rmse_pred,
            psnr_pred=psnr_pred,
...
def imageSave(pred, groundtruth, plane, save_path):
    ...
    for slice_idx in seq:
        ...
        f.savefig(
            os.path.join(
                save_path, "Plane_{}_ImageSlice_{}.png".format(plane, slice_idx + 1)
            )
        )
        plt.close()
