...
# 2x2のカーネルサイズを持つ2Dの畳み込み
def conv2x2(in_planes, out_planes, stride=2):
    return nn.Conv2d(
        in_planes, out_planes, kernel_size=2, stride=stride, padding=0, bias=False
    )


# 3x3のカーネルサイズを持つ2Dの畳み込み
    ...
    if activation == "relu":
        ...
        if activation is not None:
            raise NotImplementedError(
                "activation function [{}] is not implemented".format(activation)
            )
...
class ReconNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, gain=0.02, init_type="standard"):
        ...
        ######### representation network - convolution layers 表現ネットワーク - 畳み込み層
        ...
        ######### generation network - deconvolution layers  生成ネットワーク - 逆畳み込み層
